...
from equation import computeCost
...
def plt_intuition(x_train, y_train):
    ...
    for i in range(len(w_array)):
        tmp_w = w_array[i]
        cost[i] = computeCost(x_train, y_train, tmp_w, tmp_b)
    ...
    def func(w=150):
        ...
        cur_cost = computeCost(x_train, y_train, w, tmp_b)
...
def plt_stationary(x_train, y_train):
    ...
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i, j] = computeCost(x_train, y_train, tmp_w[i][j], tmp_b[i][j])
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ws = event.xdata
            bs = event.ydata
            cst = computeCost(self.x_train, self.y_train, ws, bs)
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 500, 5], b_range=[-500, 500, 5],
    ...
    for i in range(w0.shape[0]):
        for j in range(w0.shape[1]):
            z[i][j] = computeCost(x, y, w0[i][j], b0[i][j])
...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    for i in range(len(w_array)):
        tmp_w = w_array[i]
        cost[i] = computeCost(x_train, y_train, tmp_w, fix_b)
...
def plt_gradients(x_train, y_train, f_computeCost, f_compute_gradient):
    #===============
    #  First subplot
    #===============
    ...
    for i in range(len(w_array)):
        tmp_w = w_array[i]
        cost[i] = f_computeCost(x_train, y_train, tmp_w, fix_b)
    ...
    for tmp_w in [100, 200, 300]:
        ...
        j = f_computeCost(x_train, y_train, tmp_w, fix_b)
